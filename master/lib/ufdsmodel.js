...
function modelList(app, Model, parentDn, log, callback) {
    ...
    if (cached) {
        ...
        try {
            var items = cached.data.map(function (d) {
                return new Model(app, d);
            });
            return callback(null, items);
        } catch (e) {
            // Drop from the cache and carry on.
            log.warn(
                'error in cached data (cacheScope=\'%s\', cacheKey=\'%s\'): %s',
    ...
    log.trace({searchOpts: opts},
        '<%s> modelList: ufds search: parentDn=\'%s\'',
        Model.name, parentDn);
    ...
    app.ufdsGet(dn, function (err, entry) {
        ...
        try {
            ...
            if (err2 instanceof restify.RestError) {
                ...
                log.error(err2, 'unknown error with %s entry:',
                    Model.name, entry);
            }
