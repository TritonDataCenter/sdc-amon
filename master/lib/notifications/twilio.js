...
Twilio.prototype.sanitizeAddress = function (data) {
    ...
    if (isNaN(parseInt(stripped, 10))) {
        log.debug('Twilio.sanitizeAddress: data %s is not a phone number',
            data);
        return false;
    }
    ...
    operation.try(function (currentAttempt) {
        ...
        var req = https.request(reqOptions, function (res) {
            if (res.statusCode >= 500) {
                log.warn({res: res},
                    'twilio failure response (code %d), calling retry',
                    res.statusCode);
            ...
            res.on('end', function () {
                if (res.statusCode !== 201) {
                    log.debug({twilioResBody: res.body || '(empty)'},
                        'twilio error');
                    log.warn({res: res},
                        'failed to issue twilio notification to "%s"',
                        self.to);
                    return callback(
                        new Error(res.body ? res.body : 'UnknownError?'));
                } else {
