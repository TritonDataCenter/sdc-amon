...
Email.prototype.notify = function (options, callback) {
    ...
    if (address.indexOf('<') === -1 && (user.cn || user.sn)) {
        // TODO: handle multiple cn's or sn's. Currently you get a
        // comma-separated.
        var contactName = '';
        if (user.cn) {
            contactName += (Array.isArray(user.cn) ?
                user.cn.join(' ') : user.cn);
        }
        if (user.sn) {
            contactName += ' ' + (
    ...
    if (probe) {
        ...
        if (probe.machine) {
            ...
            if (probe.machine !== event.machine) {
                ...
                var alias = (event.machine === event.agent ?
                    event.agentAlias : null);
