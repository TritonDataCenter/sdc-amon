            ...
            function (err, req, res, obj) {
                ...
                if (obj && !err) {
                    ...
                    t.equal(obj.contacts && obj.contacts.sort().join(','),
                        ...
                        format('probe.contacts: %s === %s', obj.contacts,
                            data.contacts));
...
test('probes: list', function (t) {
    var expectedProbeNames = [
        'whistlelog', 'sanscontactfield', 'watchtestzone'];
...
test('probes: create without owning zone', function (t) {
    var probes = {
        'donotown': {
            // Just using any zone ulrich doesn't own.
            'machine': prep.otherZoneUuid,
            'type': 'log-scan',
            'config': {
                'path': '/tmp/whistle.log',
                'match': {
        ...
        'doesnotexist': {
            // some random uuid
            'machine': 'fef43adb-8152-b94c-9dd9-058247579a3d',
            'type': 'log-scan',
            'config': {
                'path': '/tmp/whistle.log',
                'match': {
        ...
        masterClient.get(path, function (err, req, res, probes) {
            ...
            async.forEach(probes, function (probe, nextProbe) {
                ...
                masterClient.del(ppath, function (pErr, pReq, pRes) {
                    ...
                    t.equal(pRes.statusCode, 204,
                        '204 response deleting ' + ppath);
                    nextProbe(pErr);
                });
